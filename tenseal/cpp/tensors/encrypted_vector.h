...
class EncryptedVector : public EncryptedTensor<plain_t, encrypted_t> {
   public:
    ...
    size_t ciphertext_size() const {
        size_t res = 0;
        for (auto& ct : this->_ciphertext) res += ct.size();
        return res;
    }
    const vector<Ciphertext>& ciphertext() const { return this->_ciphertext; }
    void ciphertext(vector<Ciphertext>&& other) { this->_ciphertext = other; }
    /**
    ...
    */
    Ciphertext diagonal_ct_vector_matmul(const PlainTensor<plain_t>& matrix) {
        ...
        this->tenseal_context()->encrypt_zero(this->_ciphertext[0].parms_id(),
                                              result);
        result.scale() = this->_ciphertext[0].scale() *
        ...
        auto worker_func = [&](size_t start, size_t end) -> Ciphertext {
            ...
            this->tenseal_context()->encrypt_zero(
                this->_ciphertext[0].parms_id(), thread_result);
            thread_result.scale() = this->_ciphertext[0].scale() *
                                    this->tenseal_context()->global_scale();
            ...
            for (size_t local_i = start; local_i < end; ++local_i) {
                ...
                if (this->_ciphertext[0].parms_id() != pt_diag.parms_id()) {
                    this->set_to_same_mod(pt_diag, _ciphertext[0]);
                }
                this->tenseal_context()->evaluator->multiply_plain(
                    this->_ciphertext[0], pt_diag, ct);
   ...
   protected:
    size_t _size;
    std::vector<Ciphertext> _ciphertext;
};

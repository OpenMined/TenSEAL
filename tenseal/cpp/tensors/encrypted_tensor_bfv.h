...
#define TENSEAL_TENSOR_ENCRYPTED_TENSOR_BFV_H
...
/**
 * EncryptedTensorBFV<plain_data_t, encrypted_t> interface - A
 *generic API for encrypted tensor operations.
 * @param plain_data_t: root plaintext datatype for representing data(int,
 *int64 etc).
...
class EncryptedTensorBFV {
   public:
    ...
    /**
     * Negates a EncryptedTensorBFV<plain_data_t, encrypted_t>.
     **/
    encrypted_t negate() const { return this->copy()->negate_inplace(); };
    virtual encrypted_t negate_inplace() = 0;
    /**
     * Compute the square of the EncryptedTensorBFV<plain_data_t, encrypted_t>.
     **/
    encrypted_t square() const { return this->copy()->square_inplace(); };
    virtual encrypted_t square_inplace() = 0;
    /**
     * Compute the power of the EncryptedTensorBFV<plain_data_t, encrypted_t> with
     ...
     * returns a new EncryptedTensorBFV<plain_data_t, encrypted_t>
     ...
     * tensors and returns a new EncryptedTensorBFV<plain_data_t, encrypted_t>
     *which is the result of either addition, substraction or multiplication in
     *an element-wise fashion. in_place functions return a reference to the same
     *object.
     **/
     ...
     *x^i
     **/
    encrypted_t polyval(const vector<int64_t>& coefficients) const {
        return this->copy()->polyval_inplace(coefficients);
    };
    virtual encrypted_t polyval_inplace(const vector<int64_t>& coefficients) = 0;
    /**
    ...
    /**
     *Recreates a new EncryptedTensorBFV<plain_data_t, encrypted_t>

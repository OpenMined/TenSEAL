...
class EncryptedTensor {
   public:
     ...
     **/
    virtual plain_container_t decrypt() const = 0;
    virtual plain_container_t decrypt(
        const shared_ptr<SecretKey>& sk) const = 0;

    /**
     * Negates a EncryptedTensor<plain_t, plain_container_t, encrypted_t>.
     **/
    encrypted_t negate() const { return this->copy()->negate_inplace(); };
    virtual encrypted_t negate_inplace() = 0;
    /**
     * Compute the square of the EncryptedTensor<plain_t, plain_container_t,
     *encrypted_t>.
     **/
    encrypted_t square() const { return this->copy()->square_inplace(); };
    virtual encrypted_t square_inplace() = 0;
    /**
     * Compute the power of the EncryptedTensor<plain_t, plain_container_t,
     *encrypted_t> with minimal multiplication depth.
     **/
     ...
     * returns a new EncryptedTensor<plain_t, plain_container_t, encrypted_t>
    ...
    encrypted_t add_plain(const plain_container_t& to_add) const {
        return this->copy()->add_plain_inplace(to_add);
    };
    virtual encrypted_t add_plain_inplace(plain_t to_add) = 0;
    virtual encrypted_t add_plain_inplace(const plain_container_t& to_add) = 0;

    encrypted_t sub_plain(plain_t to_sub) const {
        return this->copy()->sub_plain_inplace(to_sub);
    };
    encrypted_t sub_plain(const plain_container_t& to_sub) const {
        return this->copy()->sub_plain_inplace(to_sub);
    };
    virtual encrypted_t sub_plain_inplace(plain_t to_sub) = 0;
    virtual encrypted_t sub_plain_inplace(const plain_container_t& to_sub) = 0;

    encrypted_t mul_plain(plain_t to_mul) const {
        return this->copy()->mul_plain_inplace(to_mul);
    };
    encrypted_t mul_plain(const plain_container_t& to_mul) const {
        return this->copy()->mul_plain_inplace(to_mul);
    };
    virtual encrypted_t mul_plain_inplace(plain_t to_mul) = 0;
    virtual encrypted_t mul_plain_inplace(const plain_container_t& to_mul) = 0;

    encrypted_t dot_product_plain(const plain_container_t& to_mul) const {
        return this->copy()->dot_product_plain_inplace(to_mul);
    };
    virtual encrypted_t dot_product_plain_inplace(
        const plain_container_t& to_mul) = 0;
    ...
    /**
     *Recreates a new EncryptedTensor<plain_t, plain_container_t, encrypted_t>

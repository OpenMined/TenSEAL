    ...
    py::class_<Evaluator, std::shared_ptr<Evaluator>>(m, "Evaluator",
        ...
        .def("add_plain",
             [](Evaluator &e, const Ciphertext &e1, const Plaintext &e2,
                Ciphertext &dst) {
        e.add_plain(e1, e2, dst); })
        .def("sub_plain",
             [](Evaluator &e, const Ciphertext &e1, const Plaintext &e2,
                Ciphertext &dst) {
        e.sub_plain(e1, e2, dst); })
        .def("multiply_plain",
             [](Evaluator &e, const Ciphertext &enc, const Plaintext &p,
                Ciphertext &out) {
        e.multiply_plain(enc, p, out); })
        ...
        .def("multiply_many",
             [](Evaluator &e, const std::vector<Ciphertext> &enc,
                const RelinKeys &relin_keys,
                Ciphertext &dst) {
        e.multiply_many(enc, relin_keys, dst); })
        ...
        .def(
            ...
            [](Evaluator &e, const Ciphertext &enc, const RelinKeys &relin_keys,
               Ciphertext &out) {
        e.relinearize(enc, relin_keys, out); })
        ...
        .def("transform_to_ntt",
             [](Evaluator &e, const Plaintext &in, parms_id_type parms_id,
                Plaintext &dst) {
        e.transform_to_ntt(in, parms_id, dst); })
        .def("mod_switch_to_next",
        ...
        .def("mod_switch_to",
             [](Evaluator &e, const Ciphertext &enc, parms_id_type parms_id,
                Ciphertext &dst) {
        e.mod_switch_to(enc, parms_id, dst); })
        .def("mod_switch_to",
        ...
        e.apply_galois(encrypted, galois_elt, galois_keys, destination);
             })
        ...
        .def("rotate_columns",
             [](Evaluator &e, const Ciphertext &enc,
                const GaloisKeys &galois_keys,
                Ciphertext &dst) {
        e.rotate_columns(enc, galois_keys, dst); })
